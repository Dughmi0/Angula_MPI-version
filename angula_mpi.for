...
c  maxline = 2 + 1 + 100 + 100 + 100 = 303 — use 400 for safety.
      ...
      integer, parameter :: MAXPAIRS=500000
      real,   allocatable :: outbuf_local(:,:)
      integer npairs_local
      real,   allocatable :: outbuf_recv(:)
      integer, allocatable :: npairs_all(:)
      integer, allocatable :: recvcounts(:), displs(:)
      integer iline, ipair, irank, nline
      integer sendcount, totalrecv, idx0

c ------ bucket-by-central-molecule writing ------
      integer, allocatable :: bucket_counts(:)
      integer, allocatable :: bucket_offsets(:)
      integer, allocatable :: bucket_pos(:)
      real,    allocatable :: outbuf_bucketed(:)
      integer totalpairs_conf
c ------------------------------------------------
...
334       continue
        ...
        if(opd.eq.4) then
         ...
         do jat=1,natt(jm)
          do iat=1,natt(im)
           if(nomat(jat,jm).eq.'Crd'.or.nomat(iat,im).eq.'Crd')
     &      goto 335
           xc1=r(1,iat,im)
           yc1=r(2,iat,im)
           zc1=r(3,iat,im)
...
c ============================================================
c  Pack results into local output buffer
c ============================================================
...
c --- Rank 0 groups by central molecule and writes this configuration ---
      if (myrank .eq. 0) then

       totalpairs_conf = 0
       do irank=0,nprocs-1
        totalpairs_conf = totalpairs_conf + npairs_all(irank)
       end do

       if (totalpairs_conf .gt. 0) then

        allocate(bucket_counts(nm))
        allocate(bucket_offsets(nm))
        allocate(bucket_pos(nm))
        allocate(outbuf_bucketed(MAXLINE*totalpairs_conf))

        bucket_counts = 0
        bucket_offsets = 0
        bucket_pos = 0

c ------ first pass: count records for each central molecule ------
        do irank=0,nprocs-1
         do ipair=1,npairs_all(irank)
          idx0 = displs(irank) + (ipair-1)*MAXLINE
          im = int(outbuf_recv(idx0+1))
          if (im.ge.1 .and. im.le.nm) then
           bucket_counts(im) = bucket_counts(im) + 1
          end if
         end do
        end do

c ------ offsets for each bucket ------
        bucket_offsets(1) = 0
        do im=2,nm
         bucket_offsets(im)=bucket_offsets(im-1)+
     &                      MAXLINE*bucket_counts(im-1)
        end do

        do im=1,nm
         bucket_pos(im)=bucket_offsets(im)
        end do

c ------ second pass: place each record into its im bucket ------
        do irank=0,nprocs-1
         do ipair=1,npairs_all(irank)
          idx0 = displs(irank) + (ipair-1)*MAXLINE
          im = int(outbuf_recv(idx0+1))
          ...
          if (im.ge.1 .and. im.le.nm) then
           do k=1,MAXLINE
            outbuf_bucketed(bucket_pos(im)+k) = outbuf_recv(idx0+k)
           end do
           bucket_pos(im) = bucket_pos(im) + MAXLINE
          end if

         end do
        end do

c ------ write buckets in central-molecule order ------
        do im=1,nm
         do ipair=1,bucket_counts(im)
          idx0 = bucket_offsets(im) + (ipair-1)*MAXLINE

          jm = int(outbuf_bucketed(idx0+2))
          dc = outbuf_bucketed(idx0+3)

          do ii=1,numd
           dist(ii)=outbuf_bucketed(idx0+3+ii)
          end do
          do ii=1,numscal
           scalar(ii)=outbuf_bucketed(idx0+3+numd+ii)
          end do
          do ii=1,ntotang
           angbiv(ii)=outbuf_bucketed(idx0+3+numd+numscal+ii)
          end do
     ...
     &        im,jm,dc,dist(1:numd),scalar(1:numscal),angbiv(1:ntotang)
        ...
        deallocate(bucket_counts)
        deallocate(bucket_offsets)
        deallocate(bucket_pos)
        deallocate(outbuf_bucketed)
